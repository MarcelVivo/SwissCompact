...
function App() {
  ...
  async function accept(event: FormEvent<HTMLFormElement>) {
    ...
    try { const result = await request("POST", { name: form.get("name"), email: form.get("email"), confirm: form.get("confirm") === "on" }); setState(current => ({ ...current, result })); window.scrollTo({ top: 0, behavior: "smooth" }); }
