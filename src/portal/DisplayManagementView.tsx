...
export function DisplayManagementView({ displays, groups, sites, content, canEdit, canManageDevices, pairingBusyId, campaignNames, renderPreview, renderStatus, onCreate, onCampaign, onEdit, onPair, onDelete, onReload }: {
  ...
  return <section className="view display-management-view">
    ...
    {!groups.available && <div className="display-groups-unavailable"><strong>Gespeicherte Bildschirmgruppen sind momentan nicht verfügbar.</strong><span>Sie können Ihre Bildschirme weiterhin suchen, filtern und gemeinsam auswählen. Bei Bedarf hilft Ihnen der SwissCompact Support direkt weiter.</span></div>}
    ...
    <div className="display-grid managed-display-grid">{filtered.map((display) => {
      ...
      return <article className={`display-card record-card managed-display-card ${selected.has(display.id) ? "selected" : ""}`} key={display.id}>
        ...
        <div><span className="record-kind">Bildschirm · {display.orientation === "portrait" ? "Hochformat" : display.orientation === "landscape" ? "Querformat" : "Individuell"}</span><div className="record-title-line"><h3>{display.name}</h3>{renderStatus(display.status)}</div><p>{display.site?.name || "Standort noch nicht zugewiesen"}{display.area?.name ? ` · ${display.area.name}` : ""}</p>{displayGroups.length > 0 && <div className="display-card-groups">{displayGroups.map((group) => <span key={group.id}>{group.name}</span>)}</div>}<div className="record-assignment"><span>Kampagnen</span><strong>{names.length ? names.join(", ") : "Noch keine Kampagne zugeordnet"}</strong></div>{display.resolution?.width ? <small>{display.resolution.width} × {display.resolution.height} px</small> : null}<small className="creator-meta">Erstellt von {display.creator_name || "Nicht erfasst"}</small><div className="record-actions">{canEdit && <button type="button" className="assign-record-action" onClick={() => onCampaign([display.id])}>Inhalt zuweisen</button>}<a className="player-open-action" href={`/player?preview=${encodeURIComponent(display.id)}`} target="_blank" rel="noreferrer">Vorschau im Player öffnen</a>{canManageDevices && <button type="button" className="edit-record-action" onClick={() => onEdit(display)}>Bearbeiten</button>}{canManageDevices && <button type="button" className="device-link" disabled={Boolean(pairingBusyId)} onClick={() => onPair(display)}>{pairingBusyId === display.id ? "Code wird erstellt …" : display.status === "provisioning" ? "Aktivierungscode erstellen" : "Bildschirm neu verbinden"}</button>}{canManageDevices && <button type="button" className="delete-record-action" onClick={() => onDelete(display)}>Löschen</button>}</div></div>
      </article>;
    })}{!filtered.length && (displays.length === 0
      ? <div className="display-filter-empty"><strong>Noch kein Bildschirm eingerichtet</strong><span>Bereiten Sie den ersten Bildschirm vor. Danach führt Sie das Portal direkt zur Verbindung.</span>{canManageDevices && <button type="button" className="primary" onClick={onCreate}>Ersten Bildschirm vorbereiten</button>}</div>
      : <div className="display-filter-empty"><strong>Keine passenden Bildschirme</strong><span>Die aktuelle Suche oder ein Filter schliesst alle Bildschirme aus.</span><button type="button" className="secondary" onClick={() => { setSearch(""); setStatus("all"); setSiteId("all"); setOrientation("all"); setGroupId("all"); }}>Alle Filter zurücksetzen</button></div>)}</div>
