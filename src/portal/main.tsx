...
function Portal() {
  ...
  return <div className="portal" style={{ "--accent": data.profile.branding?.accent || "#d90d32" } as React.CSSProperties}>
    ...
    <main className="workspace"><header><div><div className="eyebrow">{data.profile.tenantName}</div><h1>{nav.find(([id]) => id === view)?.[1]}</h1></div><div className="profile"><span>{data.profile.displayName.slice(0,1).toUpperCase()}</span><div><strong>{data.profile.displayName}</strong><small>{labels[data.profile.role]}</small></div></div></header>
      ...
      {view === "campaigns" && <section className="view"><div className="section-title"><div><h2>Kampagnen</h2><p>Bereits vorbereitete, geplante und laufende Ausspielungen verwalten.</p></div>{canEdit && <button className="primary compact" onClick={() => { setCampaignInitialStep(1); setCreatingCampaign(true); }}><Icon name="plus"/>Auf Bildschirm anzeigen</button>}</div><div className="table-card"><div className="table-head campaign-table"><span>Ausspielung</span><span>Laufzeit</span><span>Verantwortlich</span><span>Status</span><span>Aktionen</span></div>{data.campaigns.map((item) => { const contentIds = new Set([...(item.content_links || []).flatMap((link) => link.content?.id ? [link.content.id] : []), ...(item.target_assignments || []).flatMap((assignment) => assignment.content_links.flatMap((link) => link.content?.id ? [link.content.id] : []))]); const scope = item.scope_area_id ? data.areas.find((area) => area.id === item.scope_area_id)?.name : item.scope_site_id ? data.sites.find((site) => site.id === item.scope_site_id)?.name : "Alle Standorte"; return <div className="table-row campaign-table" key={item.id}><div className="campaign-identity"><span className="record-kind">Ausspielung</span><strong>{item.name}</strong><small>{item.theme || "Automatisch eingerichtet"}</small><small>{scope || "Ohne Einordnung"} · {item.display_links?.length || 0} {(item.display_links?.length || 0) === 1 ? "Bildschirm" : "Bildschirme"} · {contentIds.size} {contentIds.size === 1 ? "Inhalt" : "Inhalte"}</small></div><span>{item.starts_at ? new Date(item.starts_at).toLocaleDateString("de-CH") : "Sofort"} – {item.ends_at ? new Date(item.ends_at).toLocaleDateString("de-CH") : "Ohne Ende"}</span><span className="creator-meta">{item.creator_name || "Nicht erfasst"}</span><Status value={item.status}/><div className="campaign-row-actions"><button className="assign-record-action" onClick={() => { setCampaignInitialStep(1); setEditingCampaign(item); }}>Bildschirme ändern</button><button className="row-action" onClick={() => { setCampaignInitialStep(4); setEditingCampaign(item); }}>{canEdit ? "Prüfen & bearbeiten" : "Ansehen"}</button>{canEdit && <button type="button" className="delete-record-action" onClick={() => requestDelete({ kind: "campaign", id: item.id, name: item.name })}>Löschen</button>}</div></div>; })}{!data.campaigns.length && <Empty>Zeigen Sie Ihren ersten Inhalt auf einem Bildschirm an.</Empty>}</div></section>}
...
function CampaignEditor({ campaign, preset, initialStep, sites, areas, content, displays, aiCredits, canEdit, canManageDevices, onContentCreated, onWaitForContentReady, onDraftCreated, onCreateDisplay, onClose, onSaved }: { campaign: Campaign | null; preset: CampaignPreset | null; initialStep: 1 | 2 | 3 | 4; sites: Site[]; areas: Area[]; content: Content[]; displays: Display[]; aiCredits: AiCredits; canEdit: boolean; canManageDevices: boolean; onContentCreated: (id: string) => Promise<Content | null>; onWaitForContentReady: (id: string) => Promise<Content>; onDraftCreated: () => Promise<PortalData | null>; onCreateDisplay: () => void; onClose: () => void; onSaved: () => void }) {
  ...
  const [scopeSiteId] = useState(campaign?.scope_site_id || "");
  const [scopeAreaId] = useState(campaign?.scope_area_id || "");
  ...
  const [scheduleMode, setScheduleMode] = useState<"now" | "scheduled">(() => campaign?.ends_at || (campaign?.starts_at && new Date(campaign.starts_at).getTime() > Date.now() + 60_000) ? "scheduled" : "now");
