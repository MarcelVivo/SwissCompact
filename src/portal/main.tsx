...
function Portal() {
  ...
  return <div className="portal" style={{ "--accent": data.profile.branding?.accent || "#d90d32" } as React.CSSProperties}>
    ...
    {(creatingCampaign || editingCampaign) && <CampaignEditor campaign={editingCampaign} preset={editingCampaign ? null : campaignPreset} sites={data.sites} areas={data.areas} content={data.content} displays={data.displays} aiCredits={data.aiCredits} canEdit={canEdit} canManageDevices={canManageDevices} onContentCreated={async (id) => {
...
function CampaignEditor({ campaign, preset, sites, areas, content, displays, aiCredits, canEdit, canManageDevices, onContentCreated, onCreateDisplay, onClose, onSaved }: { campaign: Campaign | null; preset: CampaignPreset | null; sites: Site[]; areas: Area[]; content: Content[]; displays: Display[]; aiCredits: AiCredits; canEdit: boolean; canManageDevices: boolean; onContentCreated: (id: string) => Promise<Content | null>; onCreateDisplay: () => void; onClose: () => void; onSaved: () => void }) {
  ...
  const defaultDisplayIds = campaign ? initialDisplayIds : presetDisplay ? [presetDisplay.id] : displays.length === 1 ? [displays[0].id] : [];
  const defaultPlaylist = campaign ? (initialPlaylists[0] || legacyPlaylist) : presetPlaylist;
  const [selectedDisplays, setSelectedDisplays] = useState(() => new Set(defaultDisplayIds));
  const [contentMode, setContentMode] = useState<"shared" | "individual">(initialIndividual ? "individual" : "shared");
  const [sharedPlaylist, setSharedPlaylist] = useState<PlaylistEntry[]>(defaultPlaylist);
  const [targetPlaylists, setTargetPlaylists] = useState<Record<string, PlaylistEntry[]>>(() => Object.fromEntries(defaultDisplayIds.map((id) => [id, campaign ? initialTargetPlaylists[id] || legacyPlaylist : presetPlaylist])));
  const [activeTargetId, setActiveTargetId] = useState(defaultDisplayIds[0] || "");
  const [name, setName] = useState(campaign?.name || presetContent?.title || (presetDisplay ? `Anzeige ${presetDisplay.name}` : ""));
  const [theme, setTheme] = useState(campaign?.theme || "");
  const [scopeSiteId, setScopeSiteId] = useState(campaign?.scope_site_id || presetDisplay?.site_id || (sites.length === 1 ? sites[0].id : ""));
  const [scopeAreaId, setScopeAreaId] = useState(campaign?.scope_area_id || presetDisplay?.area_id || "");
  ...
  const [step, setStep] = useState(() => presetContent && defaultDisplayIds.length ? 4 : presetContent ? 2 : presetDisplay ? 3 : 1);
  ...
  const scopeAreas = areas.filter((area) => area.active && (!scopeSiteId || area.site_id === scopeSiteId));
  ...
  function nextStep() {
    ...
    if (step === 2 && !chosenDisplays.length) { setError("Wählen Sie mindestens einen Ziel-Bildschirm aus."); return; }
    if (step === 3) {
  ...
  async function save(activate = false) {
    ...
    try {
      if (activate && hasUnapprovedContent) {
      ...
      const campaignData = { name, theme: theme || null, scopeSiteId: scopeSiteId || null, scopeAreaId: scopeAreaId || null, startsAt: startsAt ? new Date(startsAt).toISOString() : null, endsAt: endsAt ? new Date(endsAt).toISOString() : null };
      if (!campaignId) {
  ...
  function contentSelection(playlist: PlaylistEntry[], displayId: string) {
    return <div className="selection-list media-selection" ref={mediaListRef}>{availableContent.map((item) => { const index = playlist.findIndex((entry) => entry.contentId === item.id); const selected = index >= 0; return <div className={`selection-row ${selected ? "selected" : ""}`} key={item.id}><label><input type="checkbox" checked={selected} onChange={() => toggleContent(item.id, displayId)} disabled={!canEdit || locked}/><span><strong>{item.title}</strong><small>{item.content_type.toUpperCase()} · {labels[item.status] || item.status}</small></span></label>{selected && <div className="playlist-controls"><button type="button" aria-label="Nach oben" onClick={() => moveContent(index,-1,displayId)} disabled={index === 0 || locked}>↑</button><button type="button" aria-label="Nach unten" onClick={() => moveContent(index,1,displayId)} disabled={index === playlist.length-1 || locked}>↓</button><label><input type="number" min="5" max="3600" value={playlist[index].durationSeconds} onChange={(event) => setDuration(index, Number(event.target.value), displayId)} disabled={!canEdit || locked}/><span>Sek.</span></label></div>}</div>})}{!availableContent.length && <div className="wizard-empty"><strong>Noch kein Inhalt vorhanden</strong><p>Laden Sie oben ein Bild oder Video hoch oder erstellen Sie ein KI-Bild.</p></div>}</div>;
  }
  const stepLabels = ["Kampagne", "Bildschirme", "Inhalte", "Prüfen & Start"];
  const nextStepLabels = ["Weiter zu Bildschirmen", "Weiter zu Inhalten", "Weiter zur Prüfung"];
  const previousStepLabels = ["", "Zurück zur Kampagne", "Zurück zu Bildschirmen", "Zurück zu Inhalten"];
  const stepGuidance = [
    "Name, Einordnung und Laufzeit festlegen.",
    "Die Bildschirme auswählen, auf denen die Kampagne erscheinen soll.",
    "Den ausgewählten Bildschirmen Bilder oder Videos zuordnen.",
    "Die Zuordnung kontrollieren und die Kampagne starten.",
  ];
  const stepSummaries = [
    name.trim() || "Noch kein Kampagnenname",
  ...
  return <><div className="dialog-backdrop campaign-backdrop" onMouseDown={(event) => event.target === event.currentTarget && !contentDialog && onClose()}><section className="dialog campaign-editor" role="dialog" aria-modal="true"><button className="dialog-close" onClick={onClose} aria-label="Schließen">×</button><div className="campaign-editor-head"><div><div className="eyebrow">{preset ? "Schnell zuweisen" : "Kampagnenplanung"}</div><h2>{name || "Neue Kampagne"}</h2></div>{campaign && <Status value={campaign.status}/>}</div>
