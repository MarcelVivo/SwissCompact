...
import "./portal-operational-status.css";
...
function storageUploadMessage(reason: unknown): string {
  ...
  if (/maximum.*size|file.*size|too large|payload.*large|413/i.test(detail)) {
    return "Das Video ist für den direkten Upload zu gross. Bitte verkleinern Sie die Datei oder kontaktieren Sie den SwissCompact Support.";
  }
...
function Icon({ name }: { name: View | "logout" | "plus" | "install" | "more" | "ios-share" }) {
  const paths: Record<string, React.ReactNode> = {
    ...
    status: <><path d="M4 13h3l2-6 4 11 2-5h5"/><path d="M5 4h14a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2Z"/></>,
...
function PortalAccessSetup() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      if (!client) {
        if (active) { setError("Dieser Einladungslink kann momentan nicht aktiviert werden. Bitte kontaktieren Sie SwissCompact."); setState("error"); }
        return;
      }
      try {
...
function Empty({ children, action, onAction }: { children: React.ReactNode; action?: string; onAction?: () => void }) {
  return <div className="empty"><span>+</span><p>{children}</p>{action && onAction && <button type="button" className="secondary compact" onClick={onAction}>{action}</button>}</div>;
}
...
function ProjectPaymentProgress({ project }: { project: CustomerProject }) {
  const steps = [
    { amount: "50 %", label: "Anzahlung", state: project.deposit_received ? "Bezahlt" : "Offen", done: project.deposit_received },
    { amount: "30 %", label: "Montage", state: project.installation_payment_received ? "Bezahlt" : "Später fällig", done: project.installation_payment_received },
    { amount: "20 %", label: "Schlusszahlung", state: project.final_payment_received ? "Bezahlt" : "Nach Abnahme", done: project.final_payment_received },
  ];
  return <div className="project-payment"><div className="project-payment-heading"><span>Zahlungsplan</span><small>{steps.filter((step) => step.done).length} von 3 Zahlungen bestätigt</small></div><div className="project-payment-progress" aria-label="Zahlungsfortschritt">{steps.map((step, index) => <React.Fragment key={step.amount}>{index > 0 && <i className={steps[index - 1].done ? "done" : ""}/>}<span className={step.done ? "done" : ""}><b>{step.amount}</b><span>{step.label}</span><small>{step.done && <i>✓</i>}{step.state}</small></span></React.Fragment>)}</div></div>;
}

function CustomerRecordsView({ data, onRefresh, onRequestProduction }: { data: PortalData; onRefresh: () => Promise<void>; onRequestProduction: () => void }) {
  ...
  useEffect(() => {
    ...
    window.requestAnimationFrame(() => document.querySelector(".customer-project-workspace")?.scrollIntoView({ behavior: "smooth", block: "start" }));
  }, [selectedProjectId]);
  ...
  return <section className="view customer-records-view"><div className="section-title"><div><h2>Meine Vorgänge</h2><p>Von Ihrer Anfrage bis zur bezahlten Rechnung – verständlich an einem Ort.</p></div></div>
    ...
    {data.customerRecords.projects.length > 0 && <div className="project-workspace-launch"><div><b>Gemeinsame Auftragsakte</b><span>Auftrag wählen – die Akte öffnet sich sofort mit Briefing, Nachrichten, Entwürfen und Freigaben.</span></div><select aria-label="Auftrag wählen und direkt öffnen" value={selectedProjectId || ""} onChange={event => setSelectedProjectId(event.target.value || null)}><option value="">Auftrag wählen …</option>{data.customerRecords.projects.map(project => <option value={project.id} key={project.id}>{project.order_number || "Auftrag"} · {project.title}</option>)}</select></div>}
    ...
    <div className="customer-record-sections">
      {(filter === "all" || filter === "requests") && <section><header><div><span>01</span><div><small>Ihre Briefings</small><h3>Produktionsanfragen</h3></div></div><b>{data.serviceRequests.length}</b></header><div className="customer-record-list">{data.serviceRequests.map((item) => <article className="customer-record-card" key={item.id}><div className="customer-record-main"><span className="customer-record-kind">Anfrage · {item.payload?.requestTypeLabel || "Individuelle Produktion"}</span><h4>{item.title}</h4><p>{item.payload?.objective || "SwissCompact prüft Ihre Anfrage und meldet sich persönlich."}</p></div><div className="customer-record-side"><span className={`customer-record-status status-${item.payload?.serviceRequestStatus || "submitted"}`}>{serviceRequestStatusLabels[item.payload?.serviceRequestStatus || "submitted"] || "Eingegangen"}</span><small>{item.payload?.desiredDate ? `Wunschtermin ${customerDate(`${item.payload.desiredDate}T12:00:00`)}` : "Termin wird gemeinsam festgelegt"}</small></div></article>)}{!data.serviceRequests.length && <Empty action="Produktion anfragen" onAction={onRequestProduction}>Noch keine Produktionsanfrage vorhanden.</Empty>}</div></section>}
      ...
      {(filter === "all" || filter === "projects") && <section><header><div><span>03</span><div><small>Planung und Umsetzung</small><h3>Aufträge</h3></div></div><b>{data.customerRecords.projects.length}</b></header><div className="customer-record-list">{data.customerRecords.projects.map((project) => <article className="customer-record-card project-record" key={project.id}><div className="project-record-summary"><div className="customer-record-main"><span className="customer-record-kind">Auftrag · {project.order_number || "Nummer wird erstellt"}</span><h4>{project.title}</h4><p>{project.starts_on ? `Start ${customerDate(project.starts_on)}` : "Start wird gemeinsam festgelegt"} · {project.target_completion ? `Zieltermin ${customerDate(project.target_completion)}` : "Zieltermin noch offen"}</p><div className="project-responsibles"><span>Inhalt & Software <b>{project.software_owner_name}</b></span><span>Hardware & Montage <b>{project.hardware_owner_name}</b></span></div></div><div className="customer-record-side"><span className={`customer-record-status status-${project.status}`}>{projectLabels[project.status] || project.status}</span><small>Aktualisiert {customerDate(project.updated_at)}</small>{data.projectCollaboration.available && <button type="button" className="secondary" onClick={() => setSelectedProjectId(project.id)}>Auftragsakte öffnen</button>}</div></div><ProjectPaymentProgress project={project}/></article>)}{!data.customerRecords.projects.length && <Empty>Noch kein bestätigter Auftrag vorhanden.</Empty>}</div></section>}
...
function CustomerProjectWorkspace({ project, data, onRefresh, onClose }: { project: CustomerProject; data: PortalData; onRefresh: () => Promise<void>; onClose: () => void }) {
  ...
  async function run(payload: Record<string, unknown>) { setBusy(true); setError(""); setNotice(""); try { return await api<any>("/api/dashboard/records?audience=portal", { method: "POST", body: JSON.stringify(payload) }); } catch (reason) { setError((reason as Error).message); throw reason; } finally { setBusy(false); } }
  async function postMessage(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const form = event.currentTarget; const message = new FormData(form).get("message"); try { await run({ action: "post_project_message", projectId: project.id, message }); form.reset(); await onRefresh(); setNotice("Ihre Nachricht wurde direkt beim Auftrag gespeichert."); } catch { /* Fehler wird in run angezeigt. */ } }
  ...
  async function review(version: Record<string, any>, decision: "approved" | "changes_requested", feedback: string) { try { await run({ action: "review_project_deliverable", projectId: project.id, deliverableId: version.deliverable_id, versionId: version.id, decision, feedback }); await onRefresh(); setNotice(decision === "approved" ? "Der Entwurf wurde verbindlich freigegeben." : "Ihr Änderungswunsch wurde an SwissCompact gesendet."); } catch { /* Fehler wird in run angezeigt. */ } }
  async function decideRevision(revisionId: string, decision: "approved" | "declined") { try { await run({ action: "decide_project_revision_cost", revisionId, decision }); await onRefresh(); setNotice(decision === "approved" ? "Die Zusatzkosten wurden bestätigt." : "Die Zusatzkosten wurden abgelehnt."); } catch { /* Fehler wird in run angezeigt. */ } }
...
function PortalOnboarding({ currentStep, complete, campaignName, canEdit, onContinue, onStartNew, onDiscard }: { currentStep: 1 | 2 | 3 | 4; complete: boolean; campaignName?: string; canEdit: boolean; onContinue: () => void; onStartNew: () => void; onDiscard?: () => void }) {
  const steps = [
  ...
  return <section className={`portal-onboarding ${complete ? "is-complete" : ""}`} aria-label="Geführte Ersteinrichtung">
    ...
    <footer>{complete ? <><span><strong>Bereit für weitere Ausspielungen</strong><small>Bestehende Anzeigen bleiben unverändert.</small></span>{canEdit && <button type="button" className="secondary" onClick={onStartNew}>Etwas Neues anzeigen</button>}</> : <><span><strong>Nächster Schritt: {current.title}</strong><small>{campaignName ? `Die angefangene Ausspielung „${campaignName}“ wird fortgesetzt.` : current.description}</small></span>{canEdit ? <span className="portal-onboarding-actions">{campaignName && onDiscard && <button type="button" className="secondary" onClick={onDiscard}>Entwurf verwerfen</button>}<button type="button" className="primary" onClick={onContinue}>{campaignName ? "Einrichtung fortsetzen" : "Jetzt beginnen"}</button></span> : <small>Ein Inhaber oder Bearbeiter kann diesen Schritt ausführen.</small>}</>}</footer>
  </section>;
}

function contentTypeLabel(type: string): string {
  return type === "image" ? "Bild" : type === "video" ? "Video" : type === "text" ? "Text" : type === "web" ? "Web-Inhalt" : type === "composition" ? "Entwurf" : type;
}
...
function RowMenu({ label, items }: { label: string; items: RowMenuItem[] }) {
  ...
  return <div className={`row-menu ${open ? "open" : ""}`} ref={ref}>
    <button type="button" className="row-action row-menu-trigger" aria-haspopup="menu" aria-expanded={open} aria-label={label} title="Weitere Aktionen" onClick={() => setOpen((current) => !current)}><span aria-hidden="true">⋯</span></button>
    {open && <div className="row-menu-list" role="menu">{items.map((item) => <button type="button" role="menuitem" className={item.danger ? "danger" : ""} key={item.label} onClick={() => { setOpen(false); item.onSelect(); }}>{item.label}</button>)}</div>}
  </div>;
}
...
function Portal() {
  ...
  const [supportFocusId, setSupportFocusId] = useState<string | null>(null);
  ...
  const openView = (target: View) => {
    if (target === "support" && notificationCount("support") > 0) {
      const latestSupportMessage = data.support.messages
        .filter((message) => message.author_type !== "customer")
        .sort((left, right) => new Date(right.created_at).getTime() - new Date(left.created_at).getTime())[0];
      if (latestSupportMessage) setSupportFocusId(latestSupportMessage.ticket_id);
    }
    setView(target);
    if (!data.notifications.available || !notificationSections.includes(target) || notificationCount(target) < 1) return;
    const readThrough = data.generatedAt;
    setData((current) => current ? { ...current, notifications: { ...current.notifications, unreadBySection: { ...current.notifications.unreadBySection, [target]: 0 } } } : current);
    void api("/api/dashboard/records?audience=portal", {
      method: "POST",
      body: JSON.stringify({ action: "mark_notification_section_read", section: target, readThrough }),
  ...
  async function createDataRightsRequest(requestType: DataRightsRequestType, reason?: string): Promise<DataRightsActionResult> {
    const result = await api<{ requestId?: string; download?: { url: string; fileName: string } }>("/api/dashboard/records?audience=portal", { method: "POST", body: JSON.stringify({ action: "create_data_rights_request", requestType, reason }) });
    await load(false);
    return result;
  }
  async function openDataExport(requestId: string): Promise<DataRightsActionResult> {
    return api("/api/dashboard/records?audience=portal", { method: "POST", body: JSON.stringify({ action: "open_data_export", requestId }) });
  }
  ...
  async function submitSupportFeedback(messageId: string, rating: "helpful" | "not_helpful"): Promise<void> {
    await api("/api/dashboard/records?audience=portal", { method: "POST", body: JSON.stringify({ action: "submit_support_ai_feedback", messageId, rating }) });
    await load(false);
  }
  async function setContentStatus(id: string, status: "approved" | "draft") {
    try {
      ...
      await load(false);
      setNotice(status === "approved" ? "Der Inhalt ist jetzt für Bildschirme freigegeben." : "Die Freigabe wurde zurückgezogen.");
  ...
  async function changeContentArchive(id: string, action: "archive_content" | "restore_content") {
    ...
    try {
      ...
      await load(false);
      setView(action === "archive_content" ? "archive" : "content");
      setNotice(action === "archive_content" ? "Der Inhalt wurde archiviert und ist hier sicher aufbewahrt." : "Der Inhalt wurde wiederhergestellt und ist wieder unter Medien & Vorlagen verfügbar.");
    } catch (reason) {
  ...
  async function changeCampaignStatus(campaign: Campaign, action: "pause_campaign" | "activate_campaign") {
    setError("");
    try {
      await api("/api/dashboard/records?audience=portal", { method: "POST", body: JSON.stringify({ action, id: campaign.id }) });
      await load(false);
      setNotice(action === "pause_campaign" ? `„${campaign.name}“ ist pausiert und wird auf den Bildschirmen nicht mehr gezeigt.` : `„${campaign.name}“ läuft wieder auf den gewählten Bildschirmen.`);
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Der Status der Kampagne konnte nicht geändert werden"); }
  }
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      await load(false);
      setNotice(`„${deletedName}“ wurde endgültig gelöscht.`);
    } catch (reason) {
  ...
  return <div className="portal" style={{ "--accent": data.profile.branding?.accent || "#d90d32" } as React.CSSProperties}>
    ...
    <main className="workspace"><header><div><div className="eyebrow">{data.profile.tenantName}</div><h1>{nav.find(([id]) => id === view)?.[1]}</h1></div><div className="profile"><button type="button" className="pwa-install-button" data-pwa-install aria-label="App installieren"><Icon name="install"/><span>App installieren</span></button><span>{data.profile.displayName.slice(0,1).toUpperCase()}</span><div><strong>{data.profile.displayName}</strong><small>{labels[data.profile.role]}</small></div></div></header>
      {view === "overview" && <section className="view"><div className="welcome"><div><div className="eyebrow">Guten Tag, {data.profile.displayName.split(" ")[0]}</div><h2>Alles im Blick.</h2><p>Wählen Sie einen Bildschirm, fügen Sie Bild oder Video hinzu und bestimmen Sie den Zeitpunkt. Den Rest erledigt SwissCompact.</p>{canEdit && <button className="primary welcome-action" onClick={onboardingComplete ? startNewCampaign : openOnboardingStep}><Icon name="plus"/>{onboardingCampaign && !onboardingComplete ? "Einrichtung fortsetzen" : "Auf Bildschirm anzeigen"}</button>}</div><div className="pulse"><i></i>{online} von {data.displays.length} Bildschirmen online</div></div>
        <button type="button" className={`system-status-overview ${operationalCritical ? "critical" : operationalIssues ? "warning" : "healthy"}`} onClick={() => openView("status")}><b>{operationalCritical ? "×" : operationalIssues ? "!" : "✓"}</b><span><strong>{operationalCritical ? "Der Systemstatus benötigt Ihre Aufmerksamkeit" : operationalIssues ? "Es gibt Hinweise zum Betrieb" : "Systemstatus: Alles läuft"}</strong><small>{operationalIssues ? `${operationalIssues} offene ${operationalIssues === 1 ? "Meldung" : "Meldungen"} mit konkreten nächsten Schritten` : "Keine offenen technischen Warnungen"}</small></span><i>Status öffnen →</i></button>
        <button className="customer-record-overview" onClick={() => openView("records")}><div><span>Ihre Zusammenarbeit mit SwissCompact</span><strong>Offerten, Aufträge und Rechnungen</strong><small>Alle Vorgänge und geschützten Dokumente an einem Ort.</small></div><div className="customer-record-overview-counts"><span><b>{data.customerRecords.quotes.filter((quote) => ["sent", "viewed"].includes(quote.status)).length}</b> offene Offerten</span><span><b>{data.customerRecords.projects.filter((project) => !["completed", "cancelled"].includes(project.status)).length}</b> laufende Aufträge</span><i>Öffnen →</i></div></button>
        <PortalOnboarding currentStep={onboardingStep} complete={onboardingComplete} campaignName={onboardingCampaign?.name} canEdit={canEdit} onContinue={openOnboardingStep} onStartNew={startNewCampaign} onDiscard={onboardingCampaign ? () => requestDelete({ kind: "campaign", id: onboardingCampaign.id, name: onboardingCampaign.name }) : undefined}/>
      ...
      {view === "campaigns" && <section className="view">
        ...
        <div className="table-card"><div className="table-head campaign-table"><span>Ausspielung</span><span>Laufzeit</span><span>Verantwortlich</span><span>Status</span><span>Aktionen</span></div>{data.campaigns.map((item) => {
          ...
          const menuItems = [
            ...(canEdit && ["active", "scheduled"].includes(item.status) ? [{ label: "Anzeige pausieren", onSelect: () => void changeCampaignStatus(item, "pause_campaign") }] : []),
            ...(canEdit && item.status === "paused" && complete ? [{ label: "Anzeige fortsetzen", onSelect: () => void changeCampaignStatus(item, "activate_campaign") }] : []),
            ...(data.campaignVersions.available ? [{ label: "Versionen anzeigen", onSelect: () => setVersionCampaign(item) }] : []),
            ...(canEdit && complete ? [{ label: "Als Vorlage speichern", onSelect: () => setTemplateCampaign(item) }] : []),
            ...(canEdit ? [{ label: "Kampagne löschen", danger: true, onSelect: () => requestDelete({ kind: "campaign", id: item.id, name: item.name }) }] : []),
          ];
          return <div className="table-row campaign-table" key={item.id}><div className="campaign-identity"><span className="record-kind">Ausspielung</span><strong>{item.name}</strong><small>{item.theme || "Automatisch eingerichtet"}</small><small>{scope || "Ohne Einordnung"} · {item.display_links?.length || 0} {(item.display_links?.length || 0) === 1 ? "Bildschirm" : "Bildschirme"} · {contentIds.size} {contentIds.size === 1 ? "Inhalt" : "Inhalte"}</small>{!complete && !["completed", "archived"].includes(item.status) && <small className="campaign-incomplete">Noch unvollständig – {item.display_links?.length ? "Inhalt fehlt" : "Bildschirm fehlt"}</small>}</div><span>{item.starts_at ? new Date(item.starts_at).toLocaleDateString("de-CH") : "Sofort"} – {item.ends_at ? new Date(item.ends_at).toLocaleDateString("de-CH") : "Ohne Ende"}</span><span className="creator-meta">{item.creator_name || "Nicht erfasst"}</span><Status value={item.status}/><div className="campaign-row-actions"><button type="button" className="row-action is-primary" onClick={() => { setCampaignInitialStep(4); setEditingCampaign(item); }}>{canEdit ? "Prüfen & bearbeiten" : "Ansehen"}</button>{canEdit && <button type="button" className="row-action" onClick={() => { setCampaignInitialStep(1); setEditingCampaign(item); }}>Bildschirme ändern</button>}<RowMenu label={`Weitere Aktionen für ${item.name}`} items={menuItems}/></div></div>;
        })}{!data.campaigns.length && <Empty action="Schnellstart öffnen" onAction={canEdit ? startNewCampaign : undefined}>Starten Sie mit einer verständlichen Vorlage Ihre erste Anzeige.</Empty>}</div>
      </section>}
    ...
    {dialog && <CreateDialog type={dialog} initialContentType={dialog === "content" ? "image" : "composition"} waitUntilReady={waitForContentReady} onClose={() => setDialog(null)} onCreated={async (id) => { setDialog(null); await load(false); setNotice("Der Inhalt ist gespeichert. Wählen Sie jetzt den gewünschten Bildschirm."); if (id) { setCampaignPreset({ contentId: id }); setCampaignInitialStep(1); setCreatingCampaign(true); } }} />}
    {serviceRequestDialog && <ServiceRequestDialog profile={data.profile} onClose={() => setServiceRequestDialog(false)} onCreated={async () => { setServiceRequestDialog(false); await load(false); setView("records"); setNotice("Ihre Produktionsanfrage ist eingegangen und unter Meine Vorgänge sichtbar."); }} />}
    {editingContent && <ContentEditDialog content={editingContent} onClose={() => setEditingContent(null)} onUpload={() => { setEditingContent(null); setDialog("content"); }} onSaved={() => { setEditingContent(null); void load(false); setNotice("Die Änderungen am Inhalt wurden gespeichert."); }} />}
    {aiDialog && <AiImageDialog credits={data.aiCredits} canBuy={canManageDevices} checkoutNotice={creditNotice} onDismissCheckoutNotice={() => setCreditNotice(null)} onClose={() => setAiDialog(false)} onCreated={async (id) => { setAiDialog(false); await load(false); setNotice("Das KI-Bild ist gespeichert. Wählen Sie jetzt den gewünschten Bildschirm."); if (id) { setCampaignPreset({ contentId: id }); setCampaignInitialStep(1); setCreatingCampaign(true); } }} />}
    ...
    {templateCampaign && <SaveCampaignTemplateDialog campaignId={templateCampaign.id} campaignName={templateCampaign.name} onClose={() => setTemplateCampaign(null)} onSaved={async () => { await load(false); setNotice("Die Kampagne wurde als wiederverwendbare Vorlage gespeichert."); }} />}
    {versionCampaign && <CampaignVersionHistoryDialog campaign={versionCampaign} history={data.campaignVersions} canEdit={canEdit} onClose={() => setVersionCampaign(null)} onRestore={async (versionId) => { await api("/api/dashboard/records?audience=portal", { method: "POST", body: JSON.stringify({ action: "restore_campaign_version", id: versionCampaign.id, versionId }) }); await load(false); setVersionCampaign(null); setNotice("Der gewählte Stand wurde als bearbeitbarer Entwurf wiederhergestellt."); }} />}
    ...
    }} onDraftCreated={() => load(false)} onCreateDisplay={() => setDisplaySetup("campaign")} onClose={() => { setCreatingCampaign(false); setEditingCampaign(null); setCampaignPreset(null); setCampaignInitialStep(1); setDeferredPairings([]); void load(false); }} onSaved={() => { setView("campaigns"); setCreatingCampaign(false); setEditingCampaign(null); setCampaignPreset(null); setCampaignInitialStep(1); setNotice("Die Kampagne wurde gespeichert und ist jetzt in der Übersicht sichtbar."); if (deferredPairings.length) { setPairing(deferredPairings[0]); setDeferredPairings((current) => current.slice(1)); } void load(false); }} />}
    {displaySetup && <DisplaySetupDialog sites={data.sites} areas={data.areas} deferActivation={displaySetup === "campaign"} onClose={() => setDisplaySetup(null)} onCreated={(next) => { const source = displaySetup; setDisplaySetup(null); if (source === "campaign") { setDeferredPairings((current) => [...current, next]); void load(false); } else { setPairing(next); setNotice("Der Bildschirm ist vorbereitet. Verbinden Sie ihn jetzt mit dem QR-Code."); void load(false); } }} />}
    {editingDisplay && <DisplayEditDialog display={editingDisplay} sites={data.sites} areas={data.areas} onClose={() => setEditingDisplay(null)} onSaved={() => { setEditingDisplay(null); void load(false); setNotice("Die Bildschirmangaben wurden gespeichert."); }} />}
    ...
    {notice && <div className="global-message portal-success" role="status"><span>✓ {notice}</span><button type="button" onClick={() => setNotice("")} aria-label="Erfolgsmeldung schließen">×</button></div>}
...
function ServiceRequestDialog({ profile, onClose, onCreated }: { profile: PortalProfile; onClose: () => void; onCreated: () => void | Promise<void> }) {
...
function CampaignEditor({ campaign, preset, initialStep, sites, areas, content, displays, displayGroups, aiCredits, canEdit, canManageDevices, onContentCreated, onWaitForContentReady, onDraftCreated, onCreateDisplay, onClose, onSaved }: { campaign: Campaign | null; preset: CampaignPreset | null; initialStep: 1 | 2 | 3 | 4; sites: Site[]; areas: Area[]; content: Content[]; displays: Display[]; displayGroups: DisplayGroupsData; aiCredits: AiCredits; canEdit: boolean; canManageDevices: boolean; onContentCreated: (id: string) => Promise<Content | null>; onWaitForContentReady: (id: string) => Promise<Content>; onDraftCreated: () => Promise<PortalData | null>; onCreateDisplay: () => void; onClose: () => void; onSaved: () => void }) {
  ...
  const [scheduleMode, setScheduleMode] = useState<"now" | "scheduled">(() => campaign?.starts_at && new Date(campaign.starts_at).getTime() > Date.now() + 60_000 ? "scheduled" : "now");
  ...
  const endInDays = (days: number) => localDateTime(new Date(Date.now() + days * 86_400_000).toISOString());
  const [endPreset, setEndPreset] = useState<string>(() => !localDateTime(campaign?.ends_at || templateEnd) ? "" : !campaign && preset?.defaultDurationDays ? String(preset.defaultDurationDays) : "custom");
  ...
  const allTargetableSelected = targetableDisplays.length > 0 && targetableDisplays.every((display) => selectedDisplays.has(display.id));
  // Orts-Schnellauswahl lohnt sich erst bei mehreren Standorten oder Bereichen und mehr als drei Bildschirmen.
  const showHierarchyShortcuts = targetableDisplays.length > 3 && selectionHierarchyTargets.length > 2;
  ...
  const quickHint = preset?.templateName
    ? defaultDisplayIds.length && usedContentIds.length
      ...
      : `Vorlage „${preset.templateName}“ gewählt${preset.defaultDurationDays ? ` – ${preset.defaultDurationDays} Tage Laufzeit sind voreingestellt` : ""}. Wählen Sie jetzt Bildschirm und Inhalt.`
  ...
  async function nextStep() {
    ...
    if (step === 3 && scheduleMode === "now" && endsAt && new Date(endsAt) <= new Date()) { setError("Das Ende muss in der Zukunft liegen."); return; }
    setBusy(true);
    try {
  ...
  function contentSelection(playlist: PlaylistEntry[], displayId: string) {
    return <div className="selection-list media-selection" ref={mediaListRef}>{availableContent.map((item) => { const index = playlist.findIndex((entry) => entry.contentId === item.id); const selected = index >= 0; const thumb = item.poster_url || item.preview_url; return <div className={`selection-row ${selected ? "selected" : ""}`} key={item.id}><label><input type="checkbox" checked={selected} onChange={() => toggleContent(item.id, displayId)} disabled={!canEdit}/>{thumb ? <img className="selection-thumb" src={thumb} alt="" loading="lazy"/> : <span className="selection-thumb placeholder" aria-hidden="true">{contentTypeLabel(item.content_type).slice(0, 1)}</span>}<span><strong>{item.title}</strong><small>{contentTypeLabel(item.content_type)}{item.content_type === "video" && item.payload?.mediaMetadata?.durationSeconds ? ` · ${mediaDurationLabel(item.payload.mediaMetadata.durationSeconds)}` : ""}{["approved", "published"].includes(item.status) ? "" : " · wird beim Veröffentlichen freigegeben"}</small></span></label>{selected && <div className="playlist-controls"><button type="button" aria-label="Nach oben" onClick={() => moveContent(index,-1,displayId)} disabled={index === 0 || !canEdit}>↑</button><button type="button" aria-label="Nach unten" onClick={() => moveContent(index,1,displayId)} disabled={index === playlist.length-1 || !canEdit}>↓</button>{item.content_type === "video" ? <span className="playlist-auto-duration">Spielt vollständig</span> : <label><span>Anzeigedauer</span><input type="number" min="5" max="3600" value={playlist[index].durationSeconds} onChange={(event) => setDuration(index, Number(event.target.value), displayId)} disabled={!canEdit}/><span>Sek.</span></label>}</div>}</div>})}{!availableContent.length && <div className="wizard-empty"><strong>Noch kein Inhalt vorhanden</strong><p>Laden Sie oben ein Bild oder Video hoch oder erstellen Sie ein KI-Bild.</p></div>}</div>;
  }
  ...
  const stepSummaries = [
    ...
    scheduleMode === "now" ? (endsAt ? `Sofort bis ${new Date(endsAt).toLocaleDateString("de-CH")}` : "Startet sofort") : startsAt ? `Start ${new Date(startsAt).toLocaleString("de-CH", { dateStyle: "short", timeStyle: "short" })}` : "Zeitpunkt noch offen",
  ...
  const missingContentDisplays = chosenDisplays.filter((display) => !playlistFor(display.id).length);
  const readyToPublish = chosenDisplays.length > 0 && missingContentDisplays.length === 0;
  ...
  return <><div className="dialog-backdrop campaign-backdrop" onMouseDown={(event) => event.target === event.currentTarget && !contentDialog && onClose()}><section className="dialog campaign-editor" role="dialog" aria-modal="true"><button className="dialog-close" onClick={onClose} aria-label="Schließen">×</button><div className="campaign-editor-head"><div><div className="eyebrow">Geführte Ausspielung</div><h2>{campaign ? effectiveName : "Auf Bildschirm anzeigen"}</h2></div>{campaign && <Status value={campaign.status}/>}</div>
    ...
    {quickHint && !draftNotice && <div className="wizard-success quick-assignment-hint" role="status">✓ {quickHint}</div>}
    ...
    <div className={`wizard-stage ${stepCollapsed ? "is-collapsed" : ""}`}>
      {step === 1 && <section>
        <div className="wizard-stage-head"><div><span>Schritt 1 · Wo?</span><h3>Wo soll etwas erscheinen?</h3><p>{targetableDisplays.length > 1 ? "Tippen Sie die gewünschten Bildschirme an – oder wählen Sie alle auf einmal." : "Wählen Sie den Bildschirm, auf dem die Anzeige laufen soll."}</p></div><b>{chosenDisplays.length} gewählt</b></div>
        {targetableDisplays.length > 1 && <button type="button" className={`select-all-displays ${allTargetableSelected ? "selected" : ""}`} disabled={!canEdit} onClick={() => setDisplayGroup(targetableDisplays.map((display) => display.id), !allTargetableSelected)}><span aria-hidden="true">{allTargetableSelected ? "✓" : "+"}</span><strong>{allTargetableSelected ? "Alle Bildschirme sind ausgewählt" : `Alle ${targetableDisplays.length} Bildschirme auswählen`}</strong><small>{allTargetableSelected ? "Nochmals tippen, um die Auswahl aufzuheben." : "Ein Klick – die Anzeige läuft überall."}</small></button>}
        {showHierarchyShortcuts && <HierarchySelectionShortcuts targets={selectionHierarchyTargets} selectedIds={selectedDisplays} disabled={!canEdit} onChange={setDisplayGroup}/>}
        ...
        {displays.length > 0 && canManageDevices && <div className="wizard-inline-action"><span>Der gewünschte Bildschirm fehlt?</span><button type="button" className="secondary compact" onClick={onCreateDisplay}>+ Neuen Bildschirm vorbereiten</button></div>}
      </section>}
      {step === 2 && <section>
        <div className="wizard-stage-head"><div><span>Schritt 2 · Was?</span><h3>Was soll dort laufen?</h3><p>Tippen Sie Bilder oder Videos an. Alles Angetippte läuft nacheinander in einer Schleife.</p></div></div>
        {chosenDisplays.length > 1 && (contentMode === "shared" && !modeChooserOpen
          ? <button type="button" className="content-mode-toggle" onClick={() => setModeChooserOpen(true)} disabled={!canEdit}><span><strong>Überall gleich</strong><small>Alle {chosenDisplays.length} Bildschirme zeigen dieselben Inhalte.</small></span><b>Soll ein Ort etwas anderes zeigen? →</b></button>
    ...
    {error && <div className="form-error">{error}</div>}<footer className="editor-actions"><button className="secondary" onClick={onClose}>Schließen</button>{stepCollapsed ? <button className="primary" onClick={() => setStepCollapsed(false)}>Schritt öffnen</button> : <>{canEdit && isRunning && <button className="secondary danger" onClick={() => void pause()} disabled={busy}>Anzeige pausieren</button>}{step > 1 && <button className="secondary" onClick={() => { setError(""); setStepCollapsed(false); setStep((current) => current - 1); }} disabled={busy}>{previousStepLabels[step - 1]}</button>}{step < 4 && <button className="primary" onClick={nextStep}>{nextStepLabels[step - 1]}</button>}{canEdit && step === 4 && <><button className={isRunning || isClosed ? "primary" : "secondary"} onClick={() => void save(false)} disabled={busy || (isRunning && !readyToPublish)} title={isRunning && !readyToPublish ? "Für eine laufende Anzeige braucht jeder Bildschirm einen Inhalt." : undefined}>{busy ? "Wird gespeichert …" : isRunning ? "Änderungen live übernehmen" : isClosed ? "Änderungen speichern" : "Als Entwurf speichern"}</button>{!isRunning && !isClosed && <button className="primary" onClick={() => void save(true)} disabled={busy || !readyToPublish} title={readyToPublish ? undefined : "Bitte zuerst für jeden Bildschirm einen Inhalt wählen."}>{busy ? "Wird gespeichert …" : scheduleMode === "scheduled" && startsAt && new Date(startsAt) > new Date() ? "Anzeige planen" : "Jetzt veröffentlichen"}</button>}</>}</>}</footer>
...
function AiImageDialog({ credits, canBuy, checkoutNotice, onDismissCheckoutNotice, nested = false, onClose, onCreated }: { credits: AiCredits; canBuy: boolean; checkoutNotice: CreditPurchaseNotice | null; onDismissCheckoutNotice: () => void; nested?: boolean; onClose: () => void; onCreated: (id?: string) => void | Promise<void> }) {
  ...
  async function buy(packageCode: string) {
    ...
    try {
      ...
      setError(reason instanceof Error ? reason.message : "Die sichere Bezahlseite konnte nicht geöffnet werden");
      setBuying("");
    }
  }
