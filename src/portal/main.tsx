...
function Portal() {
  ...
  return <div className="portal" style={{ "--accent": data.profile.branding?.accent || "#d90d32" } as React.CSSProperties}>
    ...
    {editingContent && <ContentEditDialog content={editingContent} onClose={() => setEditingContent(null)} onSaved={() => { setEditingContent(null); void load(); }} />}
...
function DeleteDialog({ target, busy, error, onCancel, onConfirm }: { target: DeleteTarget; busy: boolean; error: string; onCancel: () => void; onConfirm: () => void }) {
  ...
  const confirmed = confirmation.trim() === target.name;
  return <div className="dialog-backdrop delete-backdrop" onMouseDown={(event) => event.target === event.currentTarget && !busy && onCancel()}><section className="dialog delete-dialog" role="alertdialog" aria-modal="true" aria-labelledby="delete-dialog-title"><button className="dialog-close" onClick={onCancel} disabled={busy} aria-label="Schließen">×</button><div className="eyebrow">{copy.eyebrow} · Bestätigung {stage} von 2</div><h2 id="delete-dialog-title">{stage === 1 ? `„${target.name}“ wirklich löschen?` : "Löschen endgültig bestätigen"}</h2>{stage === 1 ? <><p>{copy.detail}</p><div className="delete-warning"><strong>Diese Aktion kann nicht rückgängig gemacht werden.</strong><span>{copy.noun} und zugehörige Verknüpfungen werden endgültig entfernt.</span></div></> : <><p>Geben Sie zur Sicherheit den vollständigen Namen ein:</p><div className="delete-name-copy">{target.name}</div><label className="delete-confirmation-field">Name zur Bestätigung<input value={confirmation} onChange={(event) => setConfirmation(event.target.value)} autoFocus autoComplete="off" placeholder={target.name}/></label><div className="delete-warning"><strong>Zweite und letzte Sicherheitsabfrage</strong><span>Erst bei identischer Eingabe wird „{target.name}“ gelöscht.</span></div></>}{error && <div className="form-error" role="alert">{error}</div>}<footer className="delete-dialog-actions"><button type="button" className="secondary" onClick={stage === 1 ? onCancel : () => { setStage(1); setConfirmation(""); }} disabled={busy}>{stage === 1 ? "Abbrechen" : "Zurück"}</button>{stage === 1 ? <button type="button" className="delete-confirm" onClick={() => setStage(2)}>Weiter zur Sicherheitsabfrage</button> : <button type="button" className="delete-confirm" onClick={onConfirm} disabled={busy || !confirmed}>{busy ? "Wird gelöscht …" : `${copy.noun} endgültig löschen`}</button>}</footer></section></div>;
}

function ContentEditDialog({ content, onClose, onSaved }: { content: Content; onClose: () => void; onSaved: () => void }) {
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
  ...
  return <div className="dialog-backdrop" onMouseDown={(event) => event.target === event.currentTarget && !busy && onClose()}><section className="dialog record-edit-dialog" role="dialog" aria-modal="true"><button className="dialog-close" onClick={onClose} disabled={busy} aria-label="Schließen">×</button><div className="eyebrow">Inhalt bearbeiten</div><h2>{content.title}</h2><form onSubmit={submit}><label>Titel<input name="title" required autoFocus defaultValue={content.title}/></label><label>Beschreibung oder Text<textarea name="text" rows={5} defaultValue={content.payload?.text || ""} placeholder="Kurze, verständliche Beschreibung des Inhalts"/></label><div className="record-edit-readonly"><span>Inhaltstyp</span><strong>{content.content_type === "image" ? "Bild" : content.content_type === "video" ? "Video" : content.content_type}</strong><span>Status</span><strong>{labels[content.status] || content.status}</strong></div>{error && <div className="form-error" role="alert">{error}</div>}<footer className="record-edit-actions"><button type="button" className="secondary" onClick={onClose} disabled={busy}>Abbrechen</button><button className="primary" disabled={busy}>{busy ? "Wird gespeichert …" : "Änderungen speichern"}</button></footer></form></section></div>;
}
