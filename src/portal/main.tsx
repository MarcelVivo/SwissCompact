...
function Portal() {
  ...
  const nav: Array<[View,string]> = [["overview","Übersicht"],["displays","Displays"],["content","Inhalte"],["campaigns","Kampagnen"],["settings","Einstellungen"]];
  ...
  return <div className="portal" style={{ "--accent": data.profile.branding?.accent || "#d90d32" } as React.CSSProperties}>
    ...
    <main className="workspace"><header><div><div className="eyebrow">{data.profile.tenantName}</div><h1>{nav.find(([id]) => id === view)?.[1]}</h1></div><div className="profile"><span>{data.profile.displayName.slice(0,1).toUpperCase()}</span><div><strong>{data.profile.displayName}</strong><small>{labels[data.profile.role]}</small></div></div></header>
      {view === "overview" && <section className="view"><div className="welcome"><div><div className="eyebrow">Guten Tag, {data.profile.displayName.split(" ")[0]}</div><h2>Alles im Blick.</h2><p>Steuern Sie Ihre digitalen Flächen, Inhalte und Kampagnen von einem Ort aus.</p></div><div className="pulse"><i></i>{online} von {data.displays.length} Displays online</div></div>
        <div className="portal-workflow" aria-label="Einrichtung in drei Schritten"><button className={data.displays.length ? "complete" : ""} onClick={() => setView("displays")}><span>1</span><div><small>Schritt 1</small><strong>Displays verbinden</strong></div><b>{data.displays.length}</b></button><button className={data.content.length ? "complete" : ""} onClick={() => setView("content")}><span>2</span><div><small>Schritt 2</small><strong>Inhalte erstellen</strong></div><b>{data.content.length}</b></button><button className={data.campaigns.length ? "complete" : ""} onClick={() => setView("campaigns")}><span>3</span><div><small>Schritt 3</small><strong>Kampagne starten</strong></div><b>{data.campaigns.length}</b></button></div>
