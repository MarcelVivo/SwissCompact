...
function Portal() {
  ...
  return <div className="portal" style={{ "--accent": data.profile.branding?.accent || "#d90d32" } as React.CSSProperties}>
    ...
    {(creatingCampaign || editingCampaign) && <CampaignEditor campaign={editingCampaign} sites={data.sites} areas={data.areas} content={data.content} displays={data.displays} aiCredits={data.aiCredits} canEdit={canEdit} canManageDevices={canManageDevices} onContentCreated={async (id) => {
...
function CampaignEditor({ campaign, sites, areas, content, displays, aiCredits, canEdit, canManageDevices, onContentCreated, onCreateDisplay, onClose, onSaved }: { campaign: Campaign | null; sites: Site[]; areas: Area[]; content: Content[]; displays: Display[]; aiCredits: AiCredits; canEdit: boolean; canManageDevices: boolean; onContentCreated: (id: string) => Promise<Content | null>; onCreateDisplay: () => void; onClose: () => void; onSaved: () => void }) {
  ...
  const defaultDisplayIds = campaign ? initialDisplayIds : displays.length === 1 ? [displays[0].id] : [];
  ...
  const [sharedPlaylist, setSharedPlaylist] = useState<PlaylistEntry[]>(initialPlaylists[0] || legacyPlaylist);
  const [targetPlaylists, setTargetPlaylists] = useState<Record<string, PlaylistEntry[]>>(() => Object.fromEntries(defaultDisplayIds.map((id) => [id, initialTargetPlaylists[id] || legacyPlaylist])));
  const [activeTargetId, setActiveTargetId] = useState(defaultDisplayIds[0] || "");
  const [name, setName] = useState(campaign?.name || "");
  const [theme, setTheme] = useState(campaign?.theme || "");
  const [scopeSiteId, setScopeSiteId] = useState(campaign?.scope_site_id || (sites.length === 1 ? sites[0].id : ""));
  const [scopeAreaId, setScopeAreaId] = useState(campaign?.scope_area_id || "");
  ...
  return <><div className="dialog-backdrop campaign-backdrop" onMouseDown={(event) => event.target === event.currentTarget && !contentDialog && onClose()}><section className="dialog campaign-editor" role="dialog" aria-modal="true"><button className="dialog-close" onClick={onClose} aria-label="Schließen">×</button><div className="campaign-editor-head"><div><div className="eyebrow">Kampagnenplanung</div><h2>{name || "Neue Kampagne"}</h2></div>{campaign && <Status value={campaign.status}/>}</div>
