...
const presets = ["hotelLobby", "spaWellness", "guestSuite"];
const expectations = {
  hotelLobby: {
    roomSize: "small",
    floor: "stone",
    ...
    minimumDisplays: 6,
    selection: "furnishing:hotel-lobby-brand-wall",
    label: "Hotellobby",
  },
  spaWellness: {
    roomSize: "small",
    ...
    minimumFurnishings: 5,
    minimumDisplays: 2,
    selection: "furnishing:spa-wellness-treatment-bed",
    label: "Spa & Wellness",
  },
  guestSuite: {
    roomSize: "small",
    ...
    minimumFurnishings: 5,
    minimumDisplays: 2,
    selection: "furnishing:guest-suite-bed",
    label: "Gästezimmer & Suite",
  },
};
...
try {
  ...
  await page.locator('[data-showroom-theme-option="hospitality"]').click();
  await page.waitForFunction(() => (
    document.querySelector("[data-showroom]")?.getAttribute(
      "data-showroom-theme",
    ) === "hospitality"
  ...
  for (const preset of presets) {
    ...
    await page.screenshot({
      path: `/tmp/swisscompact-hospitality-${preset}.png`,
      fullPage: false,
    });
    ...
    states.push(await page.evaluate(({ expected, expectedPresets }) => {
      ...
      return {
        ...
        openingCount: Number(showroom?.getAttribute(
          "data-showroom-opening-count",
        )),
        openingDisplayCollisions: Number(showroom?.getAttribute(
          "data-showroom-opening-display-collisions",
        )),
        totalDisplays: Number(showroom?.getAttribute(
          "data-showroom-total-displays",
        )),
        ...
        title: showroom?.querySelector("[data-showroom-room-label]")
          ?.textContent?.trim(),
        visiblePresets,
        expectedPresets,
      };
