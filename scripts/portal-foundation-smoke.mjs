...
const checks = {
  ...
  logicalWorkflow: portal.includes('[["overview","Übersicht"],["records","Meine Vorgänge"],["campaigns","Kampagnen"],["displays","Bildschirme"],["content","Medien & Vorlagen"]') && portal.includes("PortalOnboarding") && ["Kampagne anlegen", "Bildschirme auswählen", "Inhalte hinzufügen", "Prüfen und starten"].every((label) => portal.includes(label)),
  resumableOnboarding: ["campaignHasContentForEveryDisplay", "onboardingCampaign", "onboardingStep", "Einrichtung fortsetzen", "Bereit für weitere Kampagnen"].every((label) => portal.includes(label)) && [".portal-onboarding-steps article.current", ".portal-onboarding-steps article.done", ".portal-onboarding-steps article.locked"].every((label) => onboardingCss.includes(label)),
