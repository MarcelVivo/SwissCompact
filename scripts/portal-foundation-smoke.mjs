...
const checks = {
  ...
  safePartnerAdvertising: ["tenant_partnerships", "tenant_partner_content_offers", "validate_partner_network_scope", "enable row level security", "service_role"].every((label) => partnerMigration.includes(label)) && ["invite_partner", "respond_partner_invitation", "create_partner_offer", "respond_partner_offer", "partnerSource", "sharedAsset"].every((label) => partnerApi.concat(records).includes(label)) && ["Partner verbinden", "Werbung anbieten", "Eingegangene Werbung prüfen", "Jetzt Bildschirm", "nie direkt veröffentlicht"].every((label) => partnerView.includes(label)) && overview.includes("loadPartnerNetwork") && portal.includes("PartnerNetworkView"),
