...
const presets = ["campusFoyer", "classroom", "libraryZone"];
const expectations = {
  campusFoyer: {
    roomSize: "compact",
    floor: "stone",
    ...
    selection: "furnishing:campus-foyer-info-kiosks",
    label: "Campus & Foyer",
  },
  classroom: {
    roomSize: "small",
    ...
    minimumDisplays: 5,
    minimumFurnishings: 5,
    selection: "furnishing:classroom-flex-tables",
    label: "Unterrichtsraum",
  },
  libraryZone: {
    roomSize: "compact",
    floor: "wood",
    minimumDisplays: 5,
    minimumFurnishings: 6,
    selection: "furnishing:library-zone-stacks",
    label: "Bibliothek & Lernzone",
  },
};
...
try {
  ...
  await page.locator('[data-showroom-theme-option="education"]').click();
  await page.waitForFunction(() => (
    document.querySelector("[data-showroom]")?.getAttribute("data-showroom-theme") === "education"
  ));
  ...
  for (const preset of presets) {
    ...
    await page.screenshot({ path: `/tmp/swisscompact-education-${preset}.png`, fullPage: false });
    ...
    states.push(await page.evaluate(({ expected, expectedPresets }) => {
      ...
      return {
        ...
        selectionKeyPresent: Boolean(showroom?.querySelector(`[data-showroom-selection-item="${expected.selection}"]`)),
  ...
  const valid = errors.length === 0
    ...
    && responsive.every((state) => (
      state.theme === "education" && !state.overflow && state.visibleRoomTypes.length === 3
    ));
