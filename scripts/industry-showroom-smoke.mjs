...
const presets = ["productionHall", "logisticsCenter", "industrialControl"];
const expectations = {
  productionHall: {
    roomSize: "compact",
    floor: "plain",
    minimumDisplays: 8,
    selection: "furnishing:production-hall-line",
  ...
  logisticsCenter: {
    ...
    floor: "stone",
    minimumDisplays: 7,
    selection: "furnishing:logistics-center-racks",
  ...
  industrialControl: {
    ...
    floor: "plain",
    minimumDisplays: 5,
    selection: "furnishing:industrial-control-desks",
    label: "Leitstand",
  },
};
...
try {
  ...
  await page.locator('[data-showroom-theme-option="industry"]').click();
  await page.waitForFunction(() => (
    document.querySelector("[data-showroom]")?.getAttribute("data-showroom-theme") === "industry"
  ));
  ...
  for (const preset of presets) {
    ...
    await page.screenshot({ path: `/tmp/swisscompact-industry-${preset}.png`, fullPage: false });
  ...
  const valid = errors.length === 0
    ...
    && responsive.every((state) => (
      state.theme === "industry" && !state.overflow && state.visibleRoomTypes.length === 3
    ));
