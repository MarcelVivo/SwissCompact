...
const presets = ["clinicReception", "waitingTreatment", "careCenter"];
const expectations = {
  clinicReception: {
    roomSize: "compact",
    floor: "stone",
    minimumDisplays: 8,
    selection: "furnishing:clinic-reception-check-in",
    label: "Klinikempfang",
  },
  waitingTreatment: {
    roomSize: "small",
    floor: "plain",
    minimumDisplays: 5,
    selection: "furnishing:waiting-treatment-bed",
    label: "Warte- & Behandlungsbereich",
  },
  careCenter: {
    roomSize: "compact",
    floor: "wood",
    minimumDisplays: 6,
    selection: "furnishing:care-center-community-table",
    label: "Pflegezentrum",
  },
};
...
try {
  ...
  await page.locator('[data-showroom-theme-option="health"]').click();
  await page.waitForFunction(() => (
    document.querySelector("[data-showroom]")?.getAttribute("data-showroom-theme") === "health"
  ));
  ...
  for (const preset of presets) {
    ...
    await page.screenshot({
      path: `/tmp/swisscompact-health-${preset}.png`,
      fullPage: false,
    });
  ...
  const valid = errors.length === 0
    ...
    && responsive.every((state) => (
      state.theme === "health" && !state.overflow && state.visibleRoomTypes.length === 3
    ));
