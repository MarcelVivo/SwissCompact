...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    const verifiedFactor = data.user.factors?.find((factor) => factor.status === "verified");
    return json({
      ok: true,
      profile,
      mfaRequired: Boolean(verifiedFactor) && profile.aal !== "aal2",
      mfaEnrollmentRequired: !verifiedFactor,
